       ...
       $(document).ready(function(){
	      ...
	      //自定义每页商品显示数目
	       $("#limitByCsid").change(function(){
		     location.href="${pageContext.request.contextPath}/product_related?csid="+$(this).attr("title")+"&pid="+$(this).attr("titlepid")+"&page=1&limit="+$(this).val();
	      }); 
       })
  </script>
 </head>
  ...
  <div class="container_12">
    <div class="grid_12">
       <div class="breadcrumbs">
	      ...
	      <span class="current">相关图书列表</span>
       </div><!-- .breadcrumbs -->
    </div><!-- .grid_12 -->
  </div><!-- .container_12 -->
  ...
  <section id="main">
    <div class="container_12">
       ...
       <div id="content" class="grid_9">
	      <h1 class="page_title">相关图书列表<span style="padding-left: 20px;font-size: 12px;color: #e0e0e0;">Related Product</span></h1>
	      
	       <div class="options">
	       	<div class="grid-list" style="width: 320px">
				<a class="list" style="background: #f5f7f9;opacity: unset;"><span>img</span></a>
				<div style="width: 245px;margin-top: 20px;margin-left: 80px;">与<font color="red">"<s:property value="model.pname"/>"</font>类似的图书</div>
		     </div><!-- .grid-list -->
		     <div class="show">
			    	每页显示记录数
			    <select id="limitByCsid" title="<s:property value="csid"/>"  titlepid="<s:property value="model.pid"/>"  name="limit" >
	    ...
	    <div class="clear"></div>
	      ...
	      <div class="pagination">
			     ...
			     <s:if test="pageBean.page != 1">
			     	<li class="prev"><a href="${pageContext.request.contextPath}/product_related?csid=<s:property value="csid"/>&pid=<s:property value="model.pid"/>&page=1&limit=<s:property value="pageBean.limit"/>">&lt;&lt;</a></li>
				    <li class="prev"><a href="${pageContext.request.contextPath}/product_related?csid=<s:property value="csid"/>&pid=<s:property value="model.pid"/>&page=<s:property value="pageBean.page - 1"/>&limit=<s:property value="pageBean.limit"/>">&lt;</a></li>
				 </s:if>   
				 <!-- 动态显示条 -->
	       		 <s:iterator begin="pageBean.start" end="pageBean.end" var="num">
	       		 	<s:if test="pageBean.page != #num">
	        	  		<li><a href="${pageContext.request.contextPath}/product_related?csid=<s:property value="csid"/>&pid=<s:property value="model.pid"/>&page=<s:property value="#num"/>&limit=<s:property value="pageBean.limit"/>" ><s:property value="#num"/></a></li>
	        	  	</s:if>
	        	  	<s:else>
				  ...
				  <s:if test="pageBean.page != pageBean.totalPage">  
				    <li class="next"><a href="${pageContext.request.contextPath}/product_related?csid=<s:property value="csid"/>&pid=<s:property value="model.pid"/>&page=<s:property value="pageBean.page + 1"/>&limit=<s:property value="pageBean.limit"/>">&gt;</a></li>
				    <li class="next"><a href="${pageContext.request.contextPath}/product_related?csid=<s:property value="csid"/>&pid=<s:property value="model.pid"/>&page=<s:property value="pageBean.totalPage"/>&limit=<s:property value="pageBean.limit"/>">&gt;&gt;</a></li>
				  </s:if>  
			     </ul>
	      </div><!-- .pagination -->
